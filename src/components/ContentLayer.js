...
const ContentLayer = ({ images, speed, className, opposite, idMap }) => {
  ...
  const sortOrder = (a, b) => {
    ...
    return opposite ? -comparison : comparison;
  };
  ...
  return (
    <div className={`content-layer ${className}`}>
      {sortedImages.map(([key, value], index) => {
        ...
        return isVideo ? (
          <video key={index} id={specificId} data-src={value} alt={`content ${index}`} loop autoPlay muted ref={setRef} />
        ) : (
          <img key={index} id={specificId} data-src={value} alt={`content ${index}`} ref={setRef} />
        );
      })}
    </div>
  );
