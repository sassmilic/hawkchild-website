...
const Events = () => {
    const [upcomingSortConfig, setUpcomingSortConfig] = useState({ key: 'date', direction: 'descending' });
    const [pastSortConfig, setPastSortConfig] = useState({ key: 'date', direction: 'descending' });
    ...
    useEffect(() => {
        const upcomingData = sortData(data.filter(event => event.upcoming === "true"), upcomingSortConfig);
        setUpcomingTableData(upcomingData);

        const pastData = sortData(data.filter(event => event.upcoming !== "true"), pastSortConfig);
